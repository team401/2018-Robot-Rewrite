 ...
 * 
 * Team 401 changes:
 * Made class abstract to allow passing in values for constants
 */
...
public abstract class Kinematics {
    ...
    public Kinematics(double trackScrubFactor, double trackWidthInches) {
        this.trackScrubFactor = trackScrubFactor;
    ...
    public Twist2d forwardKinematics(double left_wheel_delta, double right_wheel_delta) {
        double delta_v = (right_wheel_delta - left_wheel_delta) / 2 * trackScrubFactor;
        double delta_rotation = delta_v * 2 / trackWidthInches;
     ...
     */
    public RigidTransform2d integrateForwardKinematics(RigidTransform2d current_pose,
     ...
     */
    public DriveVelocity inverseKinematics(Twist2d velocity) {
        if (Math.abs(velocity.dtheta) < kEpsilon) {
            return new DriveVelocity(velocity.dx, velocity.dx);
        }
        double delta_v = trackWidthInches * velocity.dtheta / (2 * trackScrubFactor);
